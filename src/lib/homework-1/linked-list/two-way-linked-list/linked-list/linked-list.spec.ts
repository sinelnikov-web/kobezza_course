...
describe('Two way linked list implementation', () => {
    it('Push items into list', () => {
        ...
        expect(list.first?.next?.next?.prev?.value).toBe(2);
    });
    ...
    it('Insert item by index', () => {
        ...
        expect(list.first?.next?.prev?.value).toBe(1);
        expect(list.first?.next?.next?.prev?.value).toBe(5);
        expect(() => list.insert(9, 3)).toThrow('Index out of range');
    });

    it('Find item by callback', () => {
        let list = new LinkedList<{ id: number; value: string }>();
